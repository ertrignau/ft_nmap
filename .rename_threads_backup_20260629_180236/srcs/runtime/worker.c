...
static t_probe	*worker_wait_for_job(t_nmap_worker *worker)
{
	...
	while (!worker->stop && !worker->job)
		pthread_cond_wait(&worker->cond, &worker->lock);
	if (worker->stop && !worker->job)
	{
...
static void	*worker_main(void *arg)
{
	...
	while (1)
	{
		probe = worker_wait_for_job(worker);
		if (!probe)
			break ;
...
static int	worker_can_accept(t_nmap_config *config, t_nmap_worker *worker)
{
	...
	if (worker->job_pending)
		return (0);
	max_per_thread = (size_t)config->scan.max_outstanding_per_worker;
	if (max_per_thread == 0)
		max_per_thread = 1;
	return (worker->outstanding_count < max_per_thread);
}
