...
/**
 * @brief Dispatch one probe to a sender worker.
 *
 * @param config Global nmap configuration.
 * @param probe Probe selected by the scheduler.
 *
 * @return 1 if dispatched, 0 if no worker is ready now.
 */
static int	send_threaded(t_nmap_config *config, t_probe *probe)
{
	return (nmap_dispatch_probe_to_worker(config, probe));
}
